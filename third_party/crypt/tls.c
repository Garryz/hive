...
static void _init_bio(lua_State *L, struct tls_context *tls_p,
                      struct ssl_ctx *ctx_p) {
...
static void _init_client_context(lua_State *L, struct tls_context *tls_p,
                                 struct ssl_ctx *ctx_p) {
...
static void _init_server_context(lua_State *L, struct tls_context *tls_p,
                                 struct ssl_ctx *ctx_p) {
...
static struct tls_context *_check_context(lua_State *L, int idx) {
...
static struct ssl_ctx *_check_sslctx(lua_State *L, int idx) {
...
static int _ltls_context_finished(lua_State *L) {
...
static int _ltls_context_close(lua_State *L) {
    struct tls_context *tls_p = _check_context(L, 1);
    if (!tls_p->is_close) {
        SSL_free(tls_p->ssl);
        tls_p->ssl = NULL;
...
static int _bio_read(lua_State *L, struct tls_context *tls_p) {
    char outbuff[4096];
    int all_read = 0;
    int read = 0;
...
static void _bio_write(lua_State *L, struct tls_context *tls_p, const char *s,
                       size_t len) {
    char *p = (char *)s;
    size_t sz = len;
    while (sz > 0) {
...
static int _ltls_context_handshake(lua_State *L) {
    struct tls_context *tls_p = _check_context(L, 1);
    size_t slen = 0;
...
static int _ltls_context_read(lua_State *L) {
    struct tls_context *tls_p = _check_context(L, 1);
    size_t slen = 0;
...
static int _ltls_context_write(lua_State *L) {
    struct tls_context *tls_p = _check_context(L, 1);
    size_t slen = 0;
...
static int _lctx_gc(lua_State *L) {
...
static int _lctx_cert(lua_State *L) {
    struct ssl_ctx *ctx_p = _check_sslctx(L, 1);
...
static int _lctx_ciphers(lua_State *L) {
    struct ssl_ctx *ctx_p = _check_sslctx(L, 1);
...
static int lnew_ctx(lua_State *L) {
    struct ssl_ctx *ctx_p =
        (struct ssl_ctx *)lua_newuserdatauv(L, sizeof(*ctx_p), 0);
    ctx_p->ctx = SSL_CTX_new(TLSv1_2_method());
    if (!ctx_p->ctx) {
...
static int lnew_tls(lua_State *L) {
    struct tls_context *tls_p =
        (struct tls_context *)lua_newuserdatauv(L, sizeof(*tls_p), 1);
...
LUALIB_API int luaopen_crypt_tls_c(lua_State *L) {
    luaL_Reg l[] = {
        {"newctx", lnew_ctx},
        {"newtls", lnew_tls},
        {NULL, NULL},
