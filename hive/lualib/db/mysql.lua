...
local sha256_new = crypt.sha256_new
...
local function _compute_sha256_token(password, scramble)
    ...
    sha:update(message1_hash)
    sha:update(scramble)
    local message2 = sha:final()
...
local function _mysql_login(self, user, password, charset, database, on_connect)
    return function(sockchannel)
        ...
        if plugin == "caching_sha2_password" then
            token = _compute_sha256_token(password, scramble)
        else
