...
local c = require "crypt.tls.c"

local string = string

local tlshelper = {}

function tlshelper.initrequestfunc(sock, tls_ctx)
    local readfunc = socket.readfunc(sock)
    local writefunc = socket.writefunc(sock)
    return function()
...
function tlshelper.initresponsefunc(sock, tls_ctx)
    local readfunc = socket.readfunc(sock)
    local writefunc = socket.writefunc(sock)
    return function()
        while not tls_ctx:finished() do
...
function tlshelper.writefunc(sock, tls_ctx)
    local writefunc = socket.writefunc(sock)
    return function(s)
...
function tlshelper.readallfunc(sock, tls_ctx)
    return function()
        local ds = socket.readall(sock)
        local s = tls_ctx:read(ds)
        return s
    end
end
